...
async function saveEvent(topic: string, payload: any): Promise<void> {
  ...
  const option = topicConfig.mappingOptions
    ? topicConfig.mappingOptions.find((item) => item.topicEventTypeName === payload.eventTypeName)
    : undefined
  ...
  const value = option ? getValueMappings(option.fields, payload, option.autoGeneratedFields) : valueForKafkaEvent
...
const getValueMappings = (fields: { [key: string]: string }, payload: any, autoGeneratedFields?: string[]): any => {
  const value = {} as any
  ...
  autoGeneratedFields?.forEach((item: string) => {
    value[item] = UUID.generate()
  })
